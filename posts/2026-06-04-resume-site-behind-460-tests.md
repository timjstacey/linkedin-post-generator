...
Nightly jobs keep the rest current. Three of them refresh the GitHub stars on my projects grid, the CI stats on the testing page, and the test counts. A fourth reads my commits and bumps the version on its own.

I wrote up the whole build on the blog: every page, the unit-first test split, and the pipeline that gates each merge.
